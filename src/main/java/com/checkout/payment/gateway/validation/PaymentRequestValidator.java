...
public class PaymentRequestValidator {
  ...
  public void validate(PostPaymentRequest request) {
    ...
    validateExpiryDate(request.getExpiryDate(), request.getExpiryYear(), request.getExpiryMonth());
  ...
  private static void validateExpiryDate(String expiryDate, Integer expiryYear,
      Integer expiryMonth) {
    if (expiryYear == null || expiryMonth == null) {
      throw new PaymentRequestValidationException(ValidationMessages.INVALID_EXPIRY_DATE);
    }
    Pattern p = Pattern.compile("(0[1-9]|1[0-2])/(\\d{2,4})");
